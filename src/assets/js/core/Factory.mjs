...
import { randomInt, randomPointsWithMinimalDistance, randomSign, randomUnitVector, randomX, randomY } from '../utils/math.mjs';
...
export default class Factory {
  constructor() {
    this.templates = {
      ...
      bullets: [
        new Bullet({ type: 1, vertices: 4, radius: 1, color: 'green' })
      ],
    };
  }
  ...
  createBullet(x=randomX(), y=randomY(), vector) {
    const templates = this.templates.bullets;
    const index = 0; // for now, always 0
    const bullet = deepClone(templates[index]);
    ...
    bullet.vx = vector.x * spd;
    bullet.vy = vector.y * spd;
